...
const ServicesList = ({ route, navigation }) => {
  ...
  const newServReq = async () => {
    ...
    await fetch(host + `/users/userServiceList/${dataid}`, requestOptions).then((res) => res.json())
      .then((res) => {
  ...
  return (
    <>
      ...
      <View style={{ width: screenWidth, height: screenHeight }}>
        ...
        <View style={{ margin: '3%' }}>
          <Text style={styles.quick}>Services List </Text>
        </View>
        <ScrollView>
          <View>
            {/* {(serviceData != null) &&  */}
                          ...
                          <View>
                            <TouchableOpacity onPress={ () => id == null ? setId(index) : setId(null)}>
                              <View style={styles.servDataParDiv}>
                                <Text style={styles.servDataheadText}>Service Request {index + 1}</Text>
